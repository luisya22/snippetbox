...
// Snippet Define a Snippet type to hold the data for an individual snippet. Notice how the fields of the struct
...
func (m *SnippetModel) Insert(title string, content string, expires int) (int, error) {
	...
	// is the SQL statement, followed by the title, content and expiry values for the placeholder parameters. This
	// method returns a sql.Result type, which contains some basic information about what happened when the statement
	// was executed.
...
func (m *SnippetModel) Get(id int) (*Snippet, error) {
	...
	// Use teh QueryRow() method on the connection pool to execute our SQL statement, passing in the untrusted
	// id variable as the value for the placeholder parameter. This returns a pointer to a sql.Row object
	...
	// struct. Notice that the arguments to row.Scan are *pointers* to the place you want to copy the data into,
	...
	if err != nil {
		...
		// function check for that error specifically, and return our own ErrNoRecord error instead (we'll create this
		// in a moment).
...
func (m *SnippetModel) Latest() ([]*Snippet, error) {
	...
	// Use Query() method on the connection pool to execute our SQL statement. This returns a sql.Rows resultset
	...
	// returns. This defer statement should come *after* you check for an error from teh Query() method. Otherwise,
	...
	// prepares the first (and then each subsequent) row to be acted on by the rows.Scan() method. If iteration
	// over all the rows completes then the resultset automatically closes itself and frees-up the underlying
	// database connection.
	for rows.Next() {
		...
		// Again, the arguments to row.Scan() must be pointers to the place you want to copy the data into, and the
	...
	// during the iteration. It's important to call this - don't assume that a successful iteration was completed
