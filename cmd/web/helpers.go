...
func (app *application) render(w http.ResponseWriter, status int, page string, data *templateData) {
	// Retrieve the appropiate template set from the cache based on the page name (like 'home.gohtml'). If no entry
	// exists in the cache with the provided name, then create a new error and call the serverError() helper
	...
	// call our serverError() helper and then return.
	...
	// If the template is written to the buffer without any errors, we are safe to go ahead and write the HTTP
	// status code to http.ResponseWriter.
	w.WriteHeader(status)

	// Write the contents of the buffer to the http.ResponseWriter. Note: this is another time when we pass
	// our http.ResponseWriter to a function that takes an io.Writer.
	buf.WriteTo(w)
}

// Create a newTemplateData() helper, which returns a pointer to a templateData struct initialized with the
// current year. Note that we're not using the *http.Request parameter here at the moment, but we will do later
// in the book.
...
// Create a new decodePostForm() helper method. The second parameter here, dst, is the target destination that we want
